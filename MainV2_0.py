...
class Face_Recognition:
    ...
    def datasets(self):
        ...
        for filename in os.listdir(pictures_dir): # Loop through all the files in the folder
            if filename.endswith(".jpg"): # Check if the file is an image
                name = os.path.splitext(filename)[0] # Get the name without the extension
                image_path = os.path.join(pictures_dir, filename) # Get the full path of the image
                print(f"Processing image: {image_path}") # Print the name of the image
                image_of_person = face_recognition.load_image_file(image_path) # Load the image
                face_encoding = face_recognition.face_encodings(image_of_person) # Get the face encoding
    ...
    def Camera(self):
        while True:
            ...
            face_locations = face_recognition.face_locations(rgb_frame) # Get the face locations
            face_encodings = face_recognition.face_encodings(rgb_frame, face_locations) # Get the face encodings
            
            
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                name = self.faceMatch(face_encoding) # Call faceMatch function here to get the name of the person
                color = (0, 255, 0) if name != "Unknown" else (0, 0, 255) # Set the color based on whether the person is recognized
                ...
                if not is_real: # If anti_spoofing returns False
                    spoofName = "Spoof" if not anti_spoofing() else "not spoofed" # Set spoofName to "Spoof" or "not spoofed"
                    TextspoofColor = (0, 0, 255) if not anti_spoofing() else (0, 255, 0) # Set TextspoofColor to red or green
